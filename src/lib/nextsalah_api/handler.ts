import type { ILocation, IFetchPrayertimes, IErrorResponse, ApiResponse, IListFetchPrayertimes, INextSalahAPIResponse } from './interfaces';

class NextSalahAPI {
    private baseUrl: string;

    constructor(endPoint: string) {
        const apiBaseUrl = import.meta.env.VITE_API_URL || 'https://nextsalah.com/api/v1/prayertimes/';
    ...
    async get_location(data: ILocation): Promise<ApiResponse<INextSalahAPIResponse>> {
    ...
    private async sendHttpRequest<T>(url: string, method: string, body?: any): Promise<ApiResponse<T>> {
        try {
    ...
    private async handleError<T>(response: Response): Promise<ApiResponse<T>> {
        ...
        try {
            ...
            if (errorData.detail) {
                ...
                errorMessage = messages.map((m: { msg: string; }) => `${m.msg.charAt(0).toUpperCase()}${m.msg.slice(1)}`).join(', ');
            }
        } catch (error) {
            console.error(error);
        }
