<!-- <script lang="ts">
    ...
    // Check if theme supports media uploads
    $derived hasMediaSupport = data.themeTemplate?.some(field => field.type === 'file');

    // Ensure the page only loads if media is supported
    onMount(() => {
        if (!hasMediaSupport) {
    ...
    async function uploadFiles() {
        ...
        try {
            const response = await fetch('/theme?/uploadMedia', {
                method: 'POST',
                body: formData
            });
    ...
    async function deleteMedia(mediaId: string) {
        try {
            const response = await fetch(`/theme?/deleteMedia`, {
                method: 'POST',
...
<div class="container mx-auto max-w-4xl">
    {#if !hasMediaSupport}
        <div class="card bg-base-100 shadow-lg">
            <div class="card-body">
                <h2 class="card-title text-error">Media Not Supported</h2>
                <p>The current theme does not support media uploads.</p>
                <div class="card-actions justify-end">
                    <a href="/theme" class="btn btn-primary">Back to Theme Settings</a>
                </div>
            </div>
        </div>
    {:else}
